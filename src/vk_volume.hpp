#ifndef VK_VOLUME_HPP
#define VK_VOLUME_HPP
...
struct Ray {
    Ray();
    Ray (const glm::vec3 &near, const glm::vec3 &far);
    Ray(const std::pair<glm::vec3,glm::vec3> &points);
...
public:
    ...
    bool intersect(const glm::vec3 &minp, const glm::vec3 &maxp) const;
...
inline bool Frustum::intersect(const glm::vec3 &minp, const glm::vec3 &maxp) const
{
...
struct Circle {
    ...
    Circle translated(const glm::vec2 &pos);
...
struct Sphere {
    ...
    Sphere translated(const glm::vec3 &pos);
...
struct Box {
    ...
    Box translated(const glm::vec2 &pos);
    bool intersect(const glm::vec2 &point);
...
struct Volume {
    Volume();
    Volume(const glm::vec3 &min, const glm::vec3 &max);
    Volume(const Sphere &sphere);

    Volume translated(const glm::vec3 &pos) const;
    Volume scaled(const glm::vec3 &scale) const;
    Volume transformed(const glm::mat4 &transform) const;
    Volume minimumBoundingBox(const Volume &other) const;
    bool intersect(const glm::vec3 &point) const;
    bool intersect(const Volume &other) const;
    bool intersect(const vke::Frustum &f) const;
