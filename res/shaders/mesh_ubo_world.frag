...
vec3 getNormal(Material mat) {
    ...
    return vec3(hasTex) * normalize(TBN * (texture(Sampler2D[mat.bumpTex], inTex).xyz * 2.0 - 1.0)) + vec3(1 - hasTex) * normalize(inNormal);
}
...
void main()
{
    ...
    vec3 N = getNormal(mat);
    vec3 L = normalize(lightDir);
    vec3 V = normalize(viewDir);
    vec3 R = reflect(-L, N);
    vec3 halfwayDir = normalize(lightDir + viewDir);

    float diffuse = max(dot(N, L), 0.000);
    //float specular = pow(max(dot(R, V), 0.0), mat.shininess) * 0.75;
    float specular = pow(max(dot(N, halfwayDir), 0.0), mat.shininess) * 0.75;
